...
class TCPPacket {
  ...
  public TCPPacket(byte[] packet, InetAddress sender) {
    sourceAddr = sender;
    sourcePort = (((char) ((char) packet[0] << 8)) & BYTE2) | (((char) (packet[1])) & BYTE1);
    destPort = (((char) ((char) packet[2] << 8)) & BYTE2) | (((char) (packet[3])) & BYTE1);

    seqNum =
        (((int) packet[4] << 24) & BYTE4)
            | (((int) packet[5] << 16) & BYTE3)
            | (((int) packet[6] << 8) & BYTE2)
            | (((int) packet[7]) & BYTE1);
    ackNum =
        (((int) packet[8] << 24) & BYTE4)
    ...
    windowSize = (((char) ((char) packet[14] << 8)) & BYTE2) | (((char) (packet[15])) & BYTE1);
    checksum = (((char) ((char) packet[16] << 8)) & BYTE2) | (((char) (packet[17])) & BYTE1);

    // copy the data, if any
    int j = 0;
    int dataSize = (packet.length - headerLength);
    if (dataSize > 0) data = new byte[dataSize];
    else data = null;
  ...
  public byte[] getBufferPacket() {
    ...
    byte packet[];
    if (data == null) packet = new byte[20];
    else packet = new byte[20 + data.length];

    int flags = 0;
    if (ackFlag) flags = flags | BIT5;
    if (rstFlag) flags = flags | BIT3;
    if (synFlag) flags = flags | BIT2;
    if (finFlag) flags = flags | BIT1;
  ...
  public String getDebugOutput() {
    String flags = "";
    if (ackFlag) flags = "\n ackFlag IS TRUE";
    if (synFlag) flags = flags + "\n synFlag IS TRUE";
    if (finFlag) flags = flags + "\n finFlag IS TRUE";
    ...
    String toReturn =
        "\n==================="
            ...
            + "\n headerLength = "
            + headerLength;
    if (data != null) toReturn = toReturn + "\n datalen = " + data.length;
  ...
  public String toString() {
    ...
    if (synFlag) flags = flags + "S";
    else flags = flags + " ";
    if (finFlag) flags = flags + "F";
    else flags = flags + " ";
  ...
  public static void main(String args[]) {
    ...
    TCPPacket packet = new TCPPacket(12345, 23456, 1234567, 2345678, true, false, true, 56, buf);
    System.out.println(packet.getDebugOutput());
    System.out.println(packet);
    System.out.println("packet.data[3] = " + packet.data[0] + packet.data[1] + packet.data[2]);

    TCPPacket grub = new TCPPacket(packet.getBufferPacket(), packet.sourceAddr);
    System.out.println(grub.getDebugOutput());
    System.out.println(grub);
    System.out.println("grub.data[3] = " + grub.data[0] + grub.data[1] + grub.data[2]);
  }
}
